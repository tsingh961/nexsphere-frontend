...
const navItems = [
  { name: "Home", href: "/", solid: faHouse, outline: faHouse, notification: true },
  { name: "Explore", href: "/explore", solid: faSearch, outline: faSearch, bold: true },
  { name: "Notifications", href: "/notifications", solid: faBell, outline: faBellOutline },
  { name: "Messages", href: "/messages", solid: faEnvelope, outline: faEnvelopeOutline, active: true },
  { name: "Grok", href: "/grok", solid: faRobot, outline: faRobot },
  { name: "Communities", href: "/communities", solid: faUsers, outline: faUsers },
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside
        className={`h-screen w-64 bg-primaryBg text-white p-4 border-r border-borderGray flex flex-col justify-between fixed top-0 left-0 lg:flex transition-transform ${
      ...
      >
          ...
          <nav className="space-y-1">
            {navItems.map(({ name, href, solid, outline, active, bold, notification }) => (
              ...
              >
                ...
                {notification && <span className="w-2 h-2 bg-blue-500 rounded-full ml-auto"></span>}
              </Link>
            ))}
          </nav>

          {/* Post Button */}
          <button className="mt-6 flex items-center justify-center w-full bg-white text-black font-semibold py-3 rounded-full text-lg hover:bg-gray-200 transition">
        ...
        <div className="flex items-center gap-3 px-4 py-2 hover:bg-[#273340] rounded-full cursor-pointer transition">
          ...
          <FontAwesomeIcon icon={faEllipsisH} className="w-5 h-5 text-gray-400 ml-auto" />
        </div>
      </aside>

      {/* Mobile Bottom Navigation */}
      <div className="lg:hidden fixed bottom-0 w-full bg-primaryBg border-t border-[#38444D] flex justify-around py-3">
        {navItems.slice(0, 4).map(({ href, solid, outline }) => (
          <Link key={href} href={href} className="text-white p-3">
            <FontAwesomeIcon icon={pathname === href ? solid : outline} className="w-6 h-6" />
          </Link>
        ))}
